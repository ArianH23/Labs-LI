...
distanceBetweenTwoPoints([X1, Y1], [X2, Y2], D) :-
    D is sqrt(abs(X1-X2)**2+abs(Y1-Y2)**2).  % Pythagoras!
storeRouteIfBetter(Km, Route) :-
    bestRouteSoFar(BestKm, _),
    Km<BestKm,
...
heli([], AccumulatedDistance, [CurrentPoint|RouteSoFar], []) :-
    helicopterBasePoint(B),
    distanceBetweenTwoPoints(CurrentPoint, B, D),
    NextDistance is AccumulatedDistance+D,
    append([[B], [CurrentPoint|RouteSoFar]], Route),
    storeRouteIfBetter(NextDistance, Route),
    fail.
...
heli(PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter) :-
    select([From, Dest], PassengersToPickUp, PLeft),
    distanceBetweenTwoPoints(CurrentPoint, From, D),
    NextDistance is AccumulatedDistance+D,
    length(DestinationsOfPassengersInHelicopter, N),
    N<2,
        % write('xd'),
    append([[From], [CurrentPoint|RouteSoFar]], Route),
    append([[Dest], DestinationsOfPassengersInHelicopter], NewDestinations),
    heli(PLeft, NextDistance, Route, NewDestinations),
    fail.
...
heli(PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter) :-
    select(Dest, DestinationsOfPassengersInHelicopter, DLeft),
    distanceBetweenTwoPoints(CurrentPoint, Dest, D),
    NextDistance is D+AccumulatedDistance,
    append([[Dest], [CurrentPoint|RouteSoFar]], Route),
    heli(PassengersToPickUp, NextDistance, Route, DLeft),
    fail.
