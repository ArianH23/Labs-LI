...
noDoublesL([2,13]).                                                %no team gets a double on any of these rounds
...
consecutiveRounds(R,R1):- round(R), R1 is R+1, round(R1).
...
satVariable( doubleHome(S,R)   ):- round(R), team(S), !. 
satVariable( doubleAway(S,R)   ):- round(R), team(S), !. 
%%%%%%  2. Clause generation:
writeClauses:-
...
defineDoubles:-
    team(S), round(R),round(R1),
    consecutiveRounds(R,R1),
    R1> 1,
    expressAnd(doubleHome(S,R1), [home(S,R),home(S,R1)]),
    expressAnd(doubleAway(S,R1), [-home(S,R),-home(S,R1)]),
    expressOr(double(S,R1), [doubleHome(S,R1),doubleAway(S,R1)]),
    fail.
defineDoubles.

noDoubles:- 
    team(S),noDoublesL(M),
    findall(double(S,R),member(R,M),Lits),
    exactly(0,Lits),
    fail.
noDoubles.
...
% displaySol(M):- nl,write('Double '), team(T),round(R),member(double(T,R),M), write(T), write(' ronda ') ,write(R),nl,fail.
